#!/bin/bash
# =============================================================================
# fleetmcp Dry-Run Testing Suite
#
# Simulates a real user's first experience with fleetmcp as a deployed package.
# Uses only publicly available, free MCP servers.
# =============================================================================

set -e

FLEETMCP="npx fleetmcp"
PASS=0
FAIL=0
TOTAL=0
...
echo "============================================================"
echo "  fleetmcp Dry-Run Testing Suite"
echo "  Date: $(date)"
...
$FLEETMCP config remove everything 2>/dev/null || true
...
run_test \
  ...
  $FLEETMCP config add echo-local -t stdio -c bun -a "../test/echo-server.ts"
...
run_test_output_contains \
  ...
  $FLEETMCP inspect everything --filter echo

run_test_output_contains \
  "Inspect with --verbose shows full JSON schemas" '"\$schema"' \
  $FLEETMCP inspect echo-local --verbose

run_test_expect_fail \
  "Inspect nonexistent alias — errors gracefully" 2 \
  $FLEETMCP inspect does-not-exist

# ── Phase 4: Call ─────────────────────────────────────────────────

run_test_output_contains \
  "Call echo tool with key=value" "hello" \
  $FLEETMCP call echo-local echo text=hello

run_test_output_contains \
  "Call echo tool with JSON arg" "world" \
  $FLEETMCP call echo-local echo '{"text":"world"}'

run_test_output_contains \
  "Call everything: echo tool" "hello from fleetmcp" \
  $FLEETMCP call everything echo message="hello from fleetmcp"

run_test_output_contains \
  "Call everything: get-sum" "8" \
  $FLEETMCP call everything get-sum a=3 b=5

run_test_output_contains \
  "Call everything: get-tiny-image" "image" \
  $FLEETMCP call everything get-tiny-image

run_test_output_contains \
  "Piped stdin to call" "piped" \
  bash -c 'echo "{\"text\":\"piped\"}" | npx fleetmcp call echo-local echo'

# ── Phase 5: Test (Compliance) ────────────────────────────────────

run_test_output_contains \
  "Test echo-local — all checks pass" "All checks passed" \
  $FLEETMCP test echo-local

run_test_output_contains \
  "Test everything — all checks pass" "All checks passed" \
  $FLEETMCP test everything

run_test \
  "Test --ci exits 0 on pass" \
  $FLEETMCP test echo-local --ci

# ── Phase 6: Proxy ───────────────────────────────────────────────

echo -e "${CYAN}[$((TOTAL + 1))] Proxy startup + health endpoint${RESET}"
TOTAL=$((TOTAL + 1))
echo -e "${DIM}    \$ npx fleetmcp proxy (background) + curl health check${RESET}"
...
if health_output=$(curl -s http://localhost:14390/ 2>&1); then
  if echo "$health_output" | grep -q "fleetmcp-proxy"; then
    ...
    echo -e "    ${RED}FAIL (health missing fleetmcp-proxy)${RESET}"
...
run_test \
  ...
  $FLEETMCP config remove echo-local

run_test \
  "Remove everything server" \
  $FLEETMCP config remove everything
